...
def load_framework(net_name):
    ...
    if optim == 'SGD':
        if 'params' in config.keys():
            ...
            optimizer = SGD(params=module_lr, lr=config['lr'], momentum=0.9, weight_decay=0.0005, nesterov=True)
        else:
            optimizer = SGD(params=model.parameters(), lr=config['lr'], momentum=0.9, weight_decay=0.0005, nesterov=True)
    elif optim == 'Adam':
